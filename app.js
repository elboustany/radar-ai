...
(() => {
  ...
  /* ---------- the film ---------- */
  const player = $('#player'), vid = $('#filmVid'), playBtn = $('#filmPlay');
  if (player && vid) {
    playBtn.addEventListener('click', () => { player.classList.add('is-playing'); vid.setAttribute('controls', ''); vid.play(); });
    vid.addEventListener('play', () => player.classList.add('is-playing'));
    vid.addEventListener('pause', () => { if (vid.currentTime < vid.duration - 0.3) player.classList.remove('is-playing'); });
    vid.addEventListener('ended', () => { player.classList.remove('is-playing'); vid.removeAttribute('controls'); vid.currentTime = 0; });
    /* stop it when it scrolls away so the voiceover never follows you down the page */
    io((en) => { if (!en.isIntersecting && !vid.paused) vid.pause(); }, { threshold: .25 }).observe(player);
  }
